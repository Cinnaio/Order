...
class AdminBanListGui(
    ...
    player,
    54,
    JavaPlugin.getPlugin(Order::class.java).configManager.getMessage("gui-admin-ban-title", mapOf("page" to page.toString()), false)
) {

    private val slotMap = mutableMapOf<Int, OrderService.AdminBanEntry>()
    ...
    override fun render() {
        ...
        service.getBannedEntriesAsync().thenAccept { entries ->
            plugin.server.regionScheduler.execute(plugin, player.location) {
                ...
                pageEntries.forEachIndexed { idx, entry ->
                    ...
                    val item: ItemStack = if (entry.isHash && !entry.serializedData.isNullOrEmpty()) {
                        try {
                            ItemSerializer.deserialize(entry.serializedData)
                        } catch (e: Exception) {
                            if (entry.materialType != null) {
                                val mat = runCatching { Material.valueOf(entry.materialType) }.getOrNull() ?: Material.BARRIER
                                ItemStack(mat)
                            } else {
                                ItemStack(Material.BARRIER)
                            }
                        }
                    } else if (!entry.materialType.isNullOrEmpty()) {
                        val mat = runCatching { Material.valueOf(entry.materialType) }.getOrNull() ?: Material.BARRIER
                        ItemStack(mat)
                    } else {
                        ItemStack(Material.BARRIER)
                    ...
                    val name = (entry.displayName ?: entry.materialType ?: "Unknown")
                    im?.setDisplayName(name)
                    val lore = mutableListOf<String>()
                    if (entry.isHash) {
                        val shortHash = (entry.itemHash ?: "????????").take(8)
                        lore.add(config.getMessage("gui-admin-ban-type-hash", withPrefix = false))
                        lore.add(config.getMessage("gui-admin-ban-hash", mapOf("hash" to shortHash), false))
                    } else {
                        lore.add(config.getMessage("gui-admin-ban-type-material", withPrefix = false))
                        lore.add(config.getMessage("gui-admin-ban-material", mapOf("material" to (entry.materialType ?: "Unknown")), false))
                    }
                    im?.lore = lore
                    item.itemMeta = im
    ...
    override fun onClick(event: InventoryClickEvent) {
        ...
        if (slot == 45 && page > 1) {
            AdminBanListGui(player, service, page - 1).open()
            return
        }
        if (slot == 53) {
            AdminBanListGui(player, service, page + 1).open()
